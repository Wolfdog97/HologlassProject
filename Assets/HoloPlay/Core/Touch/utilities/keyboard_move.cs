...
//a FPS style keyboard movement script for the Holoplay
...
public class keyboard_move : MonoBehaviour
{
    ...
    public float keyboardLookSpeed = 90f;
    ...
    void Update()
    {
        ...
        if (Input.GetKey(KeyCode.Mouse2)) //pan around with mmb
        {
            float xMove = -Input.GetAxis("Mouse X"); 
            float yMove = -Input.GetAxis("Mouse Y");
            moveNode.localPosition += moveNode.right * moveSpeed/10 * xMove; //  the divide lets us use a single 'sensitivity' for both keyboard and mouse move
            moveNode.localPosition += moveNode.up * moveSpeed/10 * yMove;
        }
        ...
        if (Input.GetKey(KeyCode.Q))
        {
            ...
            HoloPlay.Capture.Instance.Size *= scaleMod;
        }
